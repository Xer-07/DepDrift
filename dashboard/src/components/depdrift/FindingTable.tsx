...
export function FindingCard({ finding }: { finding: Finding }) {
  return (
    <Link
      ...
      className="group block border-b border-border px-4 py-3 transition-colors last:border-b-0 hover:bg-panel-raised/60"
    >
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
        <SeverityBadge severity={finding.severity} />
        <span className="font-mono text-sm font-medium text-foreground">{finding.dependency}</span>
        ...
        {(finding.expectedVersion || finding.actualVersion) && (
          <span className="font-mono text-[11px] text-muted-foreground">
            ...
            <span className="text-medium">→</span> {finding.actualVersion ?? "—"}
          </span>
        )}
        {finding.introducedCommit && (
          <span className="inline-flex items-center gap-1 rounded border border-border bg-panel px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground">
      ...
      <p className="mt-1.5 line-clamp-2 max-w-4xl text-xs leading-relaxed text-muted-foreground">
        {finding.reasoning}
      </p>
